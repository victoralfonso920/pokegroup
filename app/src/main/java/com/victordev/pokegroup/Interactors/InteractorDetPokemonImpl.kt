...
class InteractorDetPokemonImpl(presenter: InterfaceMain.PresenterDetPokemon) : AppCompatActivity(),
    InterfaceMain.InteractorDetPokemon {
            ...
            api.getDatosGET()
                ...
                .subscribe({ result ->
                    if(result.isSuccessful){
                        val mDatosReservas= Gson().fromJson<PokemonData>(result.body().toString(), PokemonData::class.java)
                        presenter.cargarData(mDatosReservas)
                    }else{
