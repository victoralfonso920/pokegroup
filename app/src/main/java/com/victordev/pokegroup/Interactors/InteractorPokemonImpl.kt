...
import com.victordev.pokegroup.ModelSerializado.PokemonsAll
...
class InteractorPokemonImpl(presenter: InterfaceMain.PresenterPokemon) : AppCompatActivity(),
    InterfaceMain.InteractorPokemon {
    ...
    override fun CallPokemons(conexion: Boolean, api: apiGexRetrofit, ctx: Context) {
        ...
        if (conexion) {
            dialog!!.show()
            //envio de parametros metodo get
            val data = HashMap<String, String>()
            data["limit"] = "811"
            //add sufijo
            val sufijo = Utils().decrypt(Utils().SUFIJO_ALL_POKMON)
            api.getDatosGETH(sufijo,data)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe({ result ->
                    if(result.isSuccessful){
                        val mDatosReservas= Gson().fromJson<PokemonsAll>(result.body().toString(), PokemonsAll::class.java)
